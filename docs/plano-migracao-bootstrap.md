...
Antes dos lotes de telas finais, as seis fontes CSS auditadas totalizavam 75.954 bytes e 2.512 linhas. O lote do shell do chaveamento reduziu o total para 73.087 bytes e 2.422 linhas; o lote da timeline e da limpeza de seção órfã reduziu para 72.329 bytes e 2.412 linhas; este sublote visual da árvore reduz o total para 66.089 bytes e 2.232 linhas, uma redução adicional de 6.240 bytes (8,63%) e 180 linhas (7,46%). Em relação à base fixa registrada acima, a redução acumulada é de 164.653 bytes (71,36%) e 5.332 linhas (70,49%). A medição continua separada dos bundles Bootstrap e do SCSS próprio.
