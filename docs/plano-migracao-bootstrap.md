...
Revisão: 11/09/2026. Repositório: SGI. Este documento substitui o plano anterior e registra o encerramento da migração Bootstrap após a auditoria final. **Migração concluída no recorte planejado.**
...
Na revisão anterior, build, verificações PHP/JavaScript e testes isolados passaram. A matriz Docker final também cobriu a correção do fluxo offline, a semântica de partidas derivadas e os novos contratos de feedback Bootstrap: foram aprovadas 233 execuções PHPUnit, 483/483 asserções HTTP, 88/88 testes JavaScript e 51/51 cenários de navegador.
...
- hero do portal do aluno usa `bg-primary`, `rounded-4` e `text-white` nativos, removendo gradiente e pseudo-elementos decorativos;
...
- tipografia duplicada do seletor pesquisável do chaveamento foi removida, deixando trigger, busca e opções herdarem a família definida pelo tema Bootstrap;
- sombra do botão flutuante de nova turma foi migrada para o utilitário `shadow`, mantendo no CSS próprio somente sua geometria e posicionamento fixo;
- sombra ornamental do painel pesquisável do chaveamento foi removida; sua borda, empilhamento, rolagem e visibilidade continuam próprios por ser um controle com busca;
- transições da revelação contextual de ações do chaveamento e da timeline foram removidas, preservando a visibilidade por foco/hover e reduzindo movimento não essencial;
- dimensões duplicadas de `.mobile-nav` foram removidas do bundle administrativo; altura e empilhamento permanecem centralizados no SCSS compartilhado;
- os fluxos offline de encerramento passaram a confirmar o modal Bootstrap de forma explícita nos testes de navegador, preservando compatibilidade com o diálogo nativo legado;
- cards de partidas do chaveamento passaram a usar `card`, `w-100`, `overflow-hidden`, `position-relative` e `mb-3`; o CSS próprio mantém somente a posição necessária ao overlay/ações e os estados esportivos;
- equipes dentro dos cards do chaveamento passaram a usar utilitários Bootstrap de layout, tipografia, estados e badges de placar; o CSS próprio mantém somente truncamento, largura mínima, separador e dimensões do score;
- contadores de grupo do seletor pesquisável do chaveamento passaram a usar `badge`, mantendo no CSS próprio somente seu alinhamento à direita;
- marcadores de tipo do seletor pesquisável do chaveamento passaram a usar variantes `badge` Bootstrap, mantendo no CSS próprio somente seu comportamento de não encolher e a separação tipográfica;
- testes estáticos, de JavaScript e de navegador ampliados para impedir o retorno dos padrões removidos.
- referências Linux do contrato visual do acesso foram geradas após inspeção das quatro imagens (desktop/mobile, normal/erro) e validadas no Playwright.

Antes dos lotes de telas finais, as seis fontes CSS auditadas totalizavam 75.954 bytes e 2.512 linhas. O lote do shell do chaveamento reduziu o total para 73.087 bytes e 2.422 linhas; o lote da timeline e da limpeza de seção órfã reduziu para 72.329 bytes e 2.412 linhas; o sublote visual da árvore reduziu o total para 66.089 bytes e 2.232 linhas; o lote do pódio individual e da remoção do alias de animação reduziu para 65.716 bytes e 2.221 linhas; o lote de modalidades e destaques reduziu para 56.898 bytes e 1.826 linhas; o lote do layout interno do placar reduziu para 55.853 bytes e 1.812 linhas; o lote da timeline reduziu para 54.085 bytes e 1.792 linhas; o lote de alunos da turma reduziu para 41.762 bytes e 1.569 linhas; o lote do contêiner e dos controles avançados do placar reduziu para 41.145 bytes e 1.566 linhas; o lote das tabelas compartilhadas reduziu para 40.277 bytes e 1.532 linhas; o lote dos avatares de perfil reduziu para 36.802 bytes e 1.366 linhas; o lote dos avatares da navegação reduziu para 35.851 bytes e 1.338 linhas; o lote de revisão dos efeitos do placar reduziu para 34.404 bytes e 1.311 linhas; o lote de seleção e filtros reduziu para 33.981 bytes e 1.288 linhas; o lote do banner offline reduziu o total para 31.523 bytes e 1.176 linhas; o lote de limpeza de overrides reduziu para 30.208 bytes e 1.126 linhas; o lote de estados visuais reduziu para 30.032 bytes e 1.122 linhas; o lote de variants de ocorrência reduziu para 29.049 bytes e 1.086 linhas; o lote de efeitos de navegação reduziu para 28.678 bytes e 1.067 linhas; o lote de hover do chaveamento reduziu para 28.302 bytes e 1.057 linhas; o lote do modal de termos reduziu para 28.099 bytes e 1.048 linhas; o lote do seletor pesquisável e do calendário reduziu para 27.773 bytes e 1.043 linhas; o lote do gradiente do estado de posição reduziu para 27.721 bytes e 1.043 linhas; o lote do hero do portal do aluno reduziu o total para 27.010 bytes e 1.011 linhas; o lote da tipografia compartilhada do seletor reduziu para 26.890 bytes e 1.008 linhas; o lote da sombra do botão flutuante reduziu para 26.843 bytes e 1.007 linhas; o lote da sombra ornamental do painel reduziu para 26.796 bytes e 1.006 linhas; o lote das revelações contextuais reduziu para 26.743 bytes e 1.005 linhas; o lote das dimensões duplicadas da navegação reduziu para 26.666 bytes e 1.001 linhas, uma redução adicional de 77 bytes (0,29%) e 4 linhas (0,40%); o lote dos cards de partidas do chaveamento reduziu para 26.524 bytes e 995 linhas, uma redução adicional de 142 bytes (0,53%) e 6 linhas (0,60%); o lote dos utilitários das equipes do chaveamento reduziu para 25.930 bytes e 973 linhas, uma redução adicional de 594 bytes (2,24%) e 22 linhas (2,21%); o lote dos contadores do seletor reduziu para 25.792 bytes e 967 linhas, uma redução adicional de 138 bytes (0,53%) e 6 linhas (0,62%); este lote dos marcadores de tipo reduz o total para 25.561 bytes e 961 linhas, uma redução adicional de 231 bytes (0,90%) e 6 linhas (0,62%). Em relação à base fixa registrada acima, a redução acumulada é de 50.393 bytes (66,35%) e 1.551 linhas (61,74%). A medição continua separada dos bundles Bootstrap e do SCSS próprio.
...
A matriz Docker funcional foi executada após a correção final: 483/483 asserções HTTP, 88/88 testes JavaScript e 51/51 cenários de navegador, incluindo fluxos online, offline e responsivos. O contrato visual separado também foi executado com sucesso em 2/2 cenários, usando as quatro referências Linux versionadas em `tests/browser/visual-contract.spec.cjs-snapshots` junto das referências Windows.

Auditoria final concluída. Não restam lotes de migração Bootstrap previstos: a pontuação avançada, a geometria do placar/árvore e as exceções customizadas remanescentes foram revisadas e estão documentadas como regras de domínio, acessibilidade, impressão ou dimensões de interação. Agenda e chaveamento mantêm somente calendário, conectores, trilhos, overlays, números do placar, áreas de toque e estados esportivos sem equivalente Bootstrap direto. Com os testes funcionais e as referências visuais Linux aprovados, a migração global deste plano está concluída.
