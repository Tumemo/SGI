...
function apiPost(string $url, array $data, string $cookieFile): array {
    $ch = curl_init($url);
...
function apiUpload(string $url, array $postFields, string $cookieFile): array {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
...
if ($idTurma6EF > 0) {
    $pdfPath = dirname(__DIR__) . '/docs/lista_alunos/6EFB.pdf';
