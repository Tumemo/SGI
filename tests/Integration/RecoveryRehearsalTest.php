...
final class RecoveryRehearsalTest
{
    ...
    {
        echo "\n  \033[1;34m[Ensaio sintético de recuperação do baseline atual]\033[0m\n";
        ...
        try {
            ...
            $installed = (new SchemaInstaller($connection, self::schemaPath()))->install();
            self::seedSyntheticData($connection);
            $before = self::snapshot($connection);
            $baselineCount = (int) $connection->query(
                "SELECT COUNT(*) FROM sgi_migrations WHERE version = '__schema_inicial__' AND dirty = 0",
            )->fetch_column();
            $connection->close();

            $dumpVersion = self::createBackup($source, $backup);
            Assertions::assert(
                'Backup sintético contém schema, dados e triggers do baseline',
                $installed && $baselineCount === 1 && is_file($backup) && filesize($backup) > 0 && $dumpVersion !== '',
            );
            ...
            $restoredBaselineCount = (int) $connection->query(
                "SELECT COUNT(*) FROM sgi_migrations WHERE version = '__schema_inicial__' AND dirty = 0",
            )->fetch_column();
            Assertions::assert(
                'Restauração em outra base preserva matrículas, hashes e histórico',
                $restored['users'] === $before['users']
                && $restored['history'] === $before['history']
                && $restored['triggers'] === $before['triggers']
                && $restoredBaselineCount === 1
                && $restoreOutput === '',
            );
            $connection->close();
            ...
            $manifestData = [
                ...
                'source_schema' => 'database/schema-inicial.sql',
                'data_comparison' => 'users/history/triggers antes da restauração == restore do backup',
                'current_architecture_boundary' => 'a base restaurada contém o baseline atual e seu histórico técnico',
            ];
            ...
            Assertions::assert(
                'Manifesto do backup registra hash e fronteira de recuperação',
                is_file($manifest) && strlen((string) ($manifestData['backup_sha256'] ?? '')) === 64,
            );
        } finally {
    ...
    {
        return [
            'users' => $connection->query('SELECT id_usuario, matricula_usuario, interclasses_id_interclasse, senha_usuario, senha_troca_pendente FROM usuarios ORDER BY id_usuario')->fetch_all(MYSQLI_ASSOC),
    ...
    private static function seedSyntheticData(mysqli $connection): void
    {
        ...
        $statements = [
            ...
            "INSERT INTO usuarios (id_usuario, sigla_usuario, matricula_usuario, nome_usuario, senha_usuario, senha_troca_pendente, nivel_usuario, genero_usuario, data_nasc_usuario, foto_usuario, status_usuario, turmas_id_turma, interclasses_id_interclasse, chave_usuario_edicao, auth_version) VALUES (1, 'RM', 'RECOVERY-26', 'Aluno sintético', '{$hash}', 1, '3', 'MASC', '2008-01-01', '', '1', 1, 1, NULL, 1)",
    ...
    private static function runProcess(array $arguments, ?string $stdin = null, ?string $stdout = null): string
    {
        ...
        if (!is_resource($process)) {
            throw new RuntimeException('Não foi possível executar a ferramenta de recuperação.');
        }
    ...
    {
        return dirname(__DIR__, 2) . '/database/schema-inicial.sql';
    }
}
