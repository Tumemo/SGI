...
ob_start();
// Previne que qualquer aviso ou echo quebre o JSON
header('Content-Type: application/json');
$resposta = ["success" => false, "message" => ""];
try {
    $parser = new Parser();
    $pasta_pdf = StoragePaths::turmaPdfs() . DIRECTORY_SEPARATOR;
    $pasta_destino = StoragePaths::imports() . DIRECTORY_SEPARATOR;
    ...
    foreach ($todos_os_alunos as $aluno) {
        ...
        $rmNorm = \App\Modules\Participantes\Domain\MatriculaRules::normalizarRa($aluno['rm']);
        if ($rmNorm === '') {
            continue;
        }
    ...
    $arquivos_pdf = $arquivo_turma && is_file($arquivo_turma) ? [$arquivo_turma] : glob($pasta_pdf . "*.pdf");
    if (count($arquivos_pdf) > 0) {
                ...
                foreach ($linhas as $linha) {
                    ...
                    if (preg_match('/^(\d+)\s+/', $linha_limpa)) {
                        ...
                        if (!isset($alunos_por_rm[$chaveRm])) {
                            $alunos_por_rm[$chaveRm] = ['nome' => $nome_final, 'data_nascimento' => $data_nasc, 'rm' => $rm, 'genero' => $genero, 'turma' => $nome_turma];
                        }
                    }
                }
        ...
        $resultadoBD = \App\Modules\Participantes\Infrastructure\ImportacaoArquivo::importarCompetidores($conn, $id_interclasse_post, $id_categoria_post, $id_turma_post);
