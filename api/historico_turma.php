...
\App\Modules\Acesso\Presentation\Http\LegacyAccess::requerAcessoTurma($conn, $idTurma, $idInter);
...
try {
    $turmaRows = sgi_hist_q($conn, "SELECT t.*, c.nome_categoria\r\n         FROM turmas t\r\n         INNER JOIN categorias c ON c.id_categoria = t.categorias_id_categoria\r\n         WHERE t.id_turma = ? AND t.interclasses_id_interclasse = ? LIMIT 1", 'ii', [$idTurma, $idInter]);
    if (!$turmaRows) {
    ...
    $inter = $interRows[0] ?? [];
    $podio = [1 => (int) ($inter['ponto_1_lugar'] ?? 0), 2 => (int) ($inter['ponto_2_lugar'] ?? 0), 3 => (int) ($inter['ponto_3_lugar'] ?? 0)];
    /* -------------------- ARRECADAÇÃO -------------------- */
    $arrecRows = sgi_hist_q($conn, "SELECT h.*, u.nome_usuario AS registrado_por_nome\r\n         FROM historico_arrecadacoes h\r\n         LEFT JOIN usuarios u ON u.id_usuario = h.registrado_por\r\n         WHERE h.id_turma = ? AND h.id_interclasse = ? AND h.status_historico = '1'\r\n         ORDER BY h.data_registro DESC, h.id_historico DESC", 'ii', [$idTurma, $idInter]);
    $arrecItens = 0.0;
    $arrecPontos = 0;
    $arrec = [];
    foreach ($arrecRows as $r) {
        ...
        $arrec[] = ['id' => (int) $r['id_historico'], 'quantidade' => (float) $r['quantidade'], 'pontos' => (int) $r['pontos_adicionados'], 'data' => $r['data_registro'], 'registrado_por' => $r['registrado_por_nome'] ?? 'Sistema'];
    }
    /* -------------------- ESPORTES -------------------- */
    $mods = sgi_hist_q($conn, "SELECT m.id_modalidade, m.nome_modalidade, m.genero_modalidade,\r\n                tm.nome_tipo_modalidade, c.nome_categoria\r\n         FROM modalidades m\r\n         INNER JOIN tipos_modalidades tm ON tm.id_tipo_modalidade = m.tipos_modalidades_id_tipo_modalidade\r\n         INNER JOIN categorias c ON c.id_categoria = m.categorias_id_categoria\r\n         WHERE m.interclasses_id_interclasse = ? AND m.status_modalidade = '1'\r\n         ORDER BY c.nome_categoria ASC, m.nome_modalidade ASC", 'i', [$idInter]);
    $esportes = ['pontos_total' => 0, 'modalidades' => []];
    foreach ($mods as $mod) {
        $idMod = (int) $mod['id_modalidade'];
        $equipes = sgi_hist_q($conn, "SELECT id_equipe, nome_equipe, status_equipe\r\n             FROM equipes WHERE turmas_id_turma = ? AND modalidades_id_modalidade = ?", 'ii', [$idTurma, $idMod]);
        if (!$equipes) {
            continue;
        }
        ...
        $entry = ['id_modalidade' => $idMod, 'nome_modalidade' => $mod['nome_modalidade'], 'nome_categoria' => $mod['nome_categoria'], 'tipo' => $mod['nome_tipo_modalidade'], 'genero' => $mod['genero_modalidade'], 'colocacao' => null, 'pontos' => 0, 'itens' => [], 'alunos' => []];
        $alunos = sgi_hist_q($conn, "SELECT DISTINCT u.id_usuario, u.nome_usuario\r\n             FROM equipes_has_usuarios ehu\r\n             INNER JOIN usuarios u ON u.id_usuario = ehu.usuarios_id_usuario\r\n             WHERE ehu.equipes_id_equipe IN ({$inPlace})\r\n             ORDER BY u.nome_usuario ASC", $inTypes, $eqIds);
        $entry['alunos'] = array_map(static fn(array $a): array => ['id_usuario' => (int) $a['id_usuario'], 'nome_usuario' => $a['nome_usuario']], $alunos);
        ...
        if ($ehIndividual) {
            ...
            foreach ($gamesAll as $g) {
                ...
                if (!isset($porJogo[$idJ])) {
                    ...
                    $porJogo[$idJ] = ['meta' => $meta, 'nome' => $g['nome_jogo'], 'data' => $g['data_jogo'], 'partidas' => []];
                }
                $porJogo[$idJ]['partidas'][] = ['equipe' => (int) $g['equipes_id_equipe'], 'gols' => (int) $g['resultado_partida']];
            }
            ...
            foreach ($porJogo as $gj) {
                ...
                foreach ($eqIds as $eq) {
                    ...
                    $entry['pontos'] += $pts;
                    $entry['itens'][] = ['descricao' => $ehVitoria ? 'Vitória' : 'Derrota', 'detalhe' => $fase . ($nomeAdv ? ' contra ' . $nomeAdv : '') . ' · placar ' . $golsTurma . ' x ' . $golsAdv, 'pontos' => $pts];
                }
            }
        }
    ...
    /* -------------------- PENALIDADES -------------------- */
    $ocorTurma = sgi_hist_q($conn, "SELECT ot.* FROM ocorrencias_turmas ot\r\n         WHERE ot.turmas_id_turma = ?\r\n         ORDER BY ot.data_ocorrencia DESC, ot.id_ocorrencia_turma DESC", 'i', [$idTurma]);
    $ocorAluno = sgi_hist_q($conn, "SELECT o.*, u.nome_usuario\r\n         FROM ocorrencias o\r\n         INNER JOIN usuarios u ON u.id_usuario = o.usuarios_id_usuario\r\n         WHERE u.turmas_id_turma = ? AND o.status_ocorrencia = '1'\r\n         ORDER BY o.data_ocorrencia DESC, o.id_ocorrencia DESC", 'i', [$idTurma]);
    $penalidades = ['pontos_total' => 0, 'ocorrencias' => []];
    foreach ($ocorTurma as $o) {
        $pts = (int) $o['pontos_descontados'];
        $penalidades['pontos_total'] += $pts;
        $penalidades['ocorrencias'][] = ['tipo' => 'turma', 'titulo' => $o['titulo_ocorrencia'], 'descricao' => $o['descricao_ocorrencia'], 'data' => $o['data_ocorrencia'], 'aluno' => null, 'pontos' => $pts];
    }
    foreach ($ocorAluno as $o) {
        $pts = (int) $o['penalidade'];
        $penalidades['pontos_total'] += $pts;
        $penalidades['ocorrencias'][] = ['tipo' => 'aluno', 'titulo' => $o['titulo_ocorrencia'], 'descricao' => $o['descricao_ocorrencia'], 'data' => $o['data_ocorrencia'], 'aluno' => $o['nome_usuario'], 'pontos' => $pts];
    }
    echo json_encode(['success' => true, 'turma' => ['id_turma' => (int) $turma['id_turma'], 'nome_turma' => $turma['nome_turma'], 'nome_fantasia_turma' => $turma['nome_fantasia_turma'], 'turno_turma' => $turma['turno_turma'], 'nome_categoria' => $turma['nome_categoria'], 'pontuacao_turma' => (int) $turma['pontuacao_turma'], 'qtd_itens_arrecadados' => (float) $turma['qtd_itens_arrecadados']], 'interclasse' => ['id_interclasse' => (int) $inter['id_interclasse'], 'nome_interclasse' => $inter['nome_interclasse'] ?? '', 'ponto_1_lugar' => $podio[1], 'ponto_2_lugar' => $podio[2], 'ponto_3_lugar' => $podio[3], 'valor_item_arrecadacao' => (int) ($inter['valor_item_arrecadacao'] ?? 0)], 'arrecadacao' => ['itens' => round($arrecItens, 2), 'pontos' => $arrecPontos, 'registros' => $arrec], 'esportes' => $esportes, 'penalidades' => $penalidades, 'resumo' => ['arrecadacao_pontos' => $arrecPontos, 'esportes_pontos' => $esportes['pontos_total'], 'penalidades_pontos' => $penalidades['pontos_total']]], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
