...
\App\Modules\Acesso\Presentation\Http\LegacyAccess::requerNivel([3]);
...
try {
    ...
    if (!$stmtUser) {
        throw new RuntimeException('Erro ao preparar consulta de usuário: ' . $conn->error);
    }
    ...
    if (!$stmtJa) {
        throw new RuntimeException('Erro ao preparar verificação de inscrições: ' . $conn->error);
    }
    ...
    foreach ($id_equipes as $id_equipe) {
        ...
        $equipesValidas[] = ['id_equipe' => (int) $dadosEquipe['id_equipe'], 'id_modalidade' => (int) $dadosEquipe['modalidades_id_modalidade']];
    }
    ...
    $novasModalidades = array_values(array_filter($modalidadesEscolhidas, fn($id) => !isset($idsJaInscritos[$id])));
    ...
    foreach ($novasModalidades as $id_modalidade) {
        ...
        $maxEquipes = isset($dadosMod['max_equipes']) && $dadosMod['max_equipes'] !== null ? (int) $dadosMod['max_equipes'] : 0;
        ...
        $id_equipe = \App\Modules\Competicoes\Infrastructure\MysqliEquipePadraoRepository::buscarOuCriarEquipePadrao($conn, $id_modalidade, $id_turma);
        if ($id_equipe === null) {
    ...
    echo json_encode(["success" => $success, "message" => $message, "insercoes" => $insercoes, "ja_existentes" => $jaExistentes, "erros" => empty($erros) ? null : $erros]);
} catch (Throwable $e) {
