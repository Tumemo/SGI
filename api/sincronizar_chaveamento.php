...
try {
    if ($tipoModalidade === 'individual') {
        ...
        $conn->commit();
        echo json_encode(['success' => true, 'message' => 'Sincronização de modalidade individual concluída com sucesso.', 'detalhes' => $resultado], JSON_UNESCAPED_UNICODE);
        ...
        foreach ($jogos as $j) {
            ...
            $jogoExistente = \App\Modules\Competicoes\Infrastructure\MysqliChaveamentoRepository::buscarJogoPorTag($conn, $idModalidade, $nomeJogo);
            if ($jogoExistente) {
            ...
            foreach ($partidas as $p) {
                ...
                if ($partidaExistente) {
                    ...
                    $stPIns = $conn->prepare("INSERT INTO partidas (jogos_id_jogo, equipes_id_equipe, resultado_partida, status_partida) VALUES (?, ?, ?, '1')");
        ...
        foreach ($jogosConcluidosAvanco as $jc) {
            \App\Modules\Competicoes\Infrastructure\MysqliChaveamentoRepository::chaveamentoProcessarAvanco($conn, (int) $jc['id_jogo']);
        }
        // Confirma a transação
        $conn->commit();
        echo json_encode(['success' => true, 'message' => 'Sincronização de chaveamento Mata-Mata realizada com sucesso.', 'jogos_sincronizados' => $jogosProcessados], JSON_UNESCAPED_UNICODE);
    } else {
    ...
    $conn->rollback();
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Erro durante a sincronização: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
}
