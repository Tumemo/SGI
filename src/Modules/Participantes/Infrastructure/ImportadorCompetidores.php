...
final class ImportadorCompetidores
{
    ...
    {
        return \App\Shared\Storage\StoragePaths::imports() . DIRECTORY_SEPARATOR . 'info_alunos.json';
    }
    /**
     ...
     */
    public function importarDeArquivo(?string $caminhoJson = \null, ?int $idInterclasseForcado = \null, ?int $idCategoriaForcado = \null, ?int $idTurmaForcado = \null): array
    {
        ...
        if ($nomeTurmaForcada !== \null) {
            $alunos = \array_values(\array_filter($alunos, static fn ($a) => \is_array($a) && \trim((string) ($a['turma'] ?? '')) === $nomeTurmaForcada));
        }
        ...
        if ($linhas === []) {
            return ['status' => 'sucesso', 'interclasse_vinculado' => $idInterclasse, 'cadastrados' => 0, 'erros' => $prepared['avisos']];
        }
        $this->conn->begin_transaction();
        try {
            if ($idTurmaForcado !== \null && $idTurmaForcado > 0) {
                $mapaTurmas = [];
                foreach ($linhas as $l) {
            ...
            foreach ($linhas as $linha) {
                $idTurma = $mapaTurmas[$linha['turma']] ?? \null;
                if ($idTurma === \null) {
                ...
                $stmtU->bind_param('sssssiis', $linha['rm'], $linha['nome'], $senhaHash, $linha['genero'], $linha['data_nasc'], $idTurma, $idInterclasse, $chaveEdicao);
                if (!$stmtU->execute()) {
                    if ($stmtU->errno === 1062) {
                        continue;
                    }
            ...
            $stmtU->close();
            $this->conn->commit();
            return ['status' => 'sucesso', 'interclasse_vinculado' => $idInterclasse, 'cadastrados' => $cadastrados, 'erros' => $erros];
        } catch (\Throwable $e) {
            $this->conn->rollback();
            return ['status' => 'erro', 'mensagem' => 'Importação cancelada para manter o banco consistente: ' . $e->getMessage()];
        }
    }
    private function resolverInterclasseAtiva(): ?int
    {
        ...
        $row = $res->fetch_assoc();
        return isset($row['id_interclasse']) ? (int) $row['id_interclasse'] : \null;
    }
    /**
    ...
    {
        if ($forcado !== \null && $forcado > 0) {
            $st = $this->conn->prepare('SELECT id_categoria FROM categorias WHERE id_categoria = ? AND interclasses_id_interclasse = ? LIMIT 1');
            if ($st) {
    ...
    {
        $stmt = $this->conn->prepare('SELECT id_categoria FROM categorias WHERE interclasses_id_interclasse = ? ORDER BY id_categoria ASC LIMIT 1');
        if (!$stmt) {
            return \null;
        }
        ...
        $stmt->close();
        return isset($row['id_categoria']) ? (int) $row['id_categoria'] : \null;
    }
    /**
    ...
    private function prepararLinhas(array $alunos): array
    {
        ...
        foreach ($alunos as $idx => $aluno) {
            ...
            $nomeTurma = \trim((string) ($aluno['turma'] ?? ''));
            ...
            if ($nome === '' || $rm === '' || $nomeTurma === '') {
                return ['linhas' => [], 'avisos' => [], 'fatal' => 'Linha ' . ($idx + 1) . ': nome, RM ou turma ausente.'];
            }
            if ($dataNasc === \null) {
                return ['linhas' => [], 'avisos' => [], 'fatal' => 'Linha ' . ($idx + 1) . ': data de nascimento inválida ou ausente.'];
            }
            $turno = $this->normalizarTurno($aluno['turno'] ?? \null);
            $linhas[] = ['rm' => $rm, 'nome' => $nome, 'genero' => $genero, 'data_nasc' => $dataNasc, 'turma' => $nomeTurma, 'turno' => $turno];
        }
        return ['linhas' => $linhas, 'avisos' => $avisos, 'fatal' => \null];
    }
    private function normalizarTurno(mixed $turno): string
    {
        $t = \strtolower(\trim((string) $turno));
        $map = ['manhã' => 'manha', 'manha' => 'manha', 'm' => 'manha', 'tarde' => 'tarde', 't' => 'tarde', 'noite' => 'noite', 'n' => 'noite', 'integral' => 'integral', 'integral ' => 'integral'];
    ...
    private function buscarNomeTurmaPorId(int $idTurma): ?string
    {
        ...
        $row = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        return isset($row['nome_turma']) ? (string) $row['nome_turma'] : \null;
    }
    /**
