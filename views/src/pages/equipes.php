    ...
    document.getElementById('formCriarEquipe').addEventListener('submit', async (event) => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                window.location.href = `./equipe_alunos.php?id=${idInterclasse}&id_turma=${idTurma}&id_equipe=${result.id_equipe}&id_modalidade=${idModalidade}${idCategoria ? `&id_categoria=${encodeURIComponent(idCategoria)}` : ''}`;
            }, 600);
        } catch (error) {
